...
function renderSemanticFact(language: Language, fact: SemanticFact): string {
  ...
  if (fact.id.startsWith('cluster-')) {
    if (language === 'ru') return `${operation}: ${fact.subject}.${inference}${evidence}`
    if (language === 'zh') return `${operation}${fact.subject}。${inference}${evidence}`
    if (language === 'de') return `${operation}: ${fact.subject}.${inference}${evidence}`
    if (language === 'es') return `${operation}: ${fact.subject}.${inference}${evidence}`
    return `${operation}: ${fact.subject}.${inference}${evidence}`
  }
...
function dossierType(_language: Language, commit: InferredCommit): string {
  return commit.title
}
...
function buildFeatureMapNotice(transition: VersionTransition, language: Language): string {
  ...
  for (const group of transition.featureTree) {
    ...
    for (const child of group.children) {
      lines.push(`    ├─ ${child.title} — ${child.fileCount} ${language === 'ru' ? 'файл(ов)' : 'files'}, ${child.semanticFactCount} ${language === 'ru' ? 'фактов' : 'facts'}, ${child.confidence}%`)
    }
  }
